...
- **23 Fix now** — factually wrong narrative/conceptual content. Cut into a remediation PR off `feature/cyoda-go-init`.
- **5 Reframe post-#80** — content stays but needs a "From the binary" callout into `cyoda help <topic>` once help ships.
- **6 Delete post-#80** — pure reference duplication (Trino surface pages, time/message trigger claims) that disappears when help lands.
- **32 clarity suggestions** — fold into the Fix-now remediation PR on a lighter review bar.

Site-wide sweeps (see [`cross-cutting.md`](cross-cutting.md) §1–4) are the most efficient fix vector:

1. **Scope contamination** — Cloud-only features in OSS pages (Trino, time/message workflow triggers).
...
| [concepts/apis-and-surfaces](pages/concepts__apis-and-surfaces.md) | 1 | 0 | 1 | 1 |
...
| [build/analytics-with-sql](pages/build__analytics-with-sql.md) | 0 | 0 | 3 | 0 |
| [build/client-compute-nodes](pages/build__client-compute-nodes.md) | 0 | 0 | 0 | 2 |
| [build/testing-with-digital-twins](pages/build__testing-with-digital-twins.md) | 1 | 0 | 0 | 1 |
...
| [reference/trino](pages/reference__trino.md) | 1 | 0 | 1 | 1 |
...
1. **Fix-now remediation PR** — cut a branch off `feature/cyoda-go-init`, action every **Fix now** finding (23) and every accepted clarity suggestion (32). Site-wide sweeps first (single grep-and-replace across pages), then per-page fixes for content that doesn't fit a sweep. Review, merge.
2. **Wait for cyoda-go #80** — when `help.tar.gz` / `help.json` release assets ship, run the post-#80 alignment test per spec §"Post-#80 alignment test":
   - **Check 1:** ledger ↔ `help.json` diff (green / amber / red per surface).
   - **Check 2:** per-page Reframe/Delete dispatch (11 items pre-enumerated).
3. **Post-#80 reframe PR** — cyoda-docs #69. Import `help.tar.gz` at build time, add "From the binary" callouts where they apply, drop `awaiting-upstream` banners, delete pure-duplication pages (analytics-with-sql, trino).
